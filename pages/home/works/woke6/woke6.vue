<template>
	<view class="boxxxxx">
		...
		<u-search style='width: 80%;' class='search' placeholder="客户,工程师" @change='init(1)' :show-action='false' v-model="search">
		...
		<view v-for="(item,index) in Data" class="item" :key="index" @click="Assign(item)" :style="[{background:(item.DataType == 2?'#fbffb5':'#fff')}]">
			...
			<view class="bottom">
				<text class="OrderCode">{{item.OrderCode}}</text><text>{{item.CreateDate}}</text>
			</view>
			<view class="bottom">
