<template>
	<view class="box">
		...
		<view v-if="Data.EquDtos" v-for="(item,index) in Data.EquDtos" class="basicinformation">
			...
			<view class="text">
				<view class="span">寄回：</view> <span :style="{color:(item.Recovery ?'#007AFF':'')}">
					{{item.Recovery ? '需要' : '不需要' }}</span>
			</view>
			<view class="textx">
		...
		<view v-if=" Data.Dto && Data.Dto.State == 7 && DataType != 1" class="cu-form-group align-start">
			<view class="title red">有需要删除/作废的设备吗？<br>如果有的话，请拍照，图片中需要包含清晰的英之杰二维码</view>
			<div class="ShangChuanImg" @tap='previewImage(item.Image)' v-for="(item,index) in this.Data.DelOldEquDtos">
				<u-image width="100px" height="100px" border-radius='10px' :src="item.Image">
				</u-image>
			</div>

			<UploadPictures :count='1' :sourceType="sourceType" @ReturnBase64='ReturnBase64' :icon='"camera"'>
			</UploadPictures>
		</view>
