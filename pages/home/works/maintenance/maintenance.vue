<template>
	<view class="box">
		...
		<view class="basicinformation" v-if="MSSODtoState">
			<view class="text">
				<view class="span">耗材状态：</view>
				{{ MSSODto.MSSOStateStr }}
			</view>
				...
				<view v-for="(items, indexs) in item.EquPhotos" :key="indexs" class="EquPhotos">
					...
					<view class="pushimg">
						<view @tap="addimgleright(items, 1)" class="left right">
							<u-image width="100%" height="100%" :src="items.Before" mode="widthFix" border-radius="10px" @click="previewImg(items, 0)" v-if="items.Before">
								<view slot="error" style="font-size: 24rpx">加载失败</view>
							</u-image>

							<UploadPictures
								:count="1"
								:news="items.PhotoId + '%%1'"
						...
						<view @tap="addimgleright(items, 2)" class="left">
							<u-image width="100%" height="100%" :src="items.After" mode="widthFix" border-radius="10px" @click="previewImg(items, 1)" v-if="items.After">
								<view slot="error" style="font-size: 24rpx">加载失败</view>
							</u-image>

							<UploadPictures
								:count="1"
								:news="items.PhotoId + '%%2'"
								:sourceType="sourceType"
								@ReturnBase64="ReturnBase64"
								:icon="'camera'"
								:text="'拍摄上传'"
		...
		<u-button v-if="this.state == 1 || this.state == 8 || this.state == 9" :disabled="GOstatus" class="button" @tap="zhi">指 派</u-button>
		<!-- 接受 ？？ 拒绝-->
