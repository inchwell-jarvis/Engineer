<template>
	<view class="box">
		...
		<view class="inputArea assd">
			<!-- <input v-model="account" placeholder="请输入账号" class="inputClass" confirm-type='' /> -->
		...
		<view class="inputArea">
			<u-button type="primary" size="medium" @tap="inter(2)" class='logon'>登 录</u-button>
		</view>
		<!-- 测试使用账号 -->
...
<script>
	export default {
		...
		methods: {
			...
			gcs() { //工程师测试账号
				this.account = 'gcs', this.Password = '123456',
					this.Logo()
			},
			qyjl() { //区域经理测试账号
				this.account = 'quyujingli', this.Password = '888888',
					this.Logo()
			},
			// 提供测试环境使用
			Logo() {
			...
			inter(inx) {
				// 判断是否想要进入测试环境
				if (this.account == 'gototest' || this.account == 'goto test' || this.account == 'Goto test' || this
					.account == 'Gototest') {
					this.mode = true
				...
				if (inx == 1) {
					PPFace.faceliveness(config, result => {
						_self.msg = new Date().getHours() + ':' + new Date().getMinutes() + '  ' + JSON.stringify(
								result) + '<br/>' +
							_self.msg;
